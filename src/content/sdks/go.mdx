...
```bash
go get github.com/chirrop/chirrop-go
```
...
_ = client.Log(context.Background(), chirrop.Log{
    Agent: "openclaw.main",
...
for _, eventDef := range events {
    if eventDef.Agent == "openclaw.main" && eventDef.Event == "task.duration_ms" {
        _, _ = client.GetEventAnalytics(context.Background(), eventDef.EventID, chirrop.AnalyticsWindowQuery{
            View:     "window",
            Period:   "1h",
            Previous: "previous_window",
        })

        destination, _ := client.CreateDestination(context.Background(), chirrop.Destination{
            Channel: "slack",
...
client, err := chirrop.NewClient(chirrop.Options{
    Key:        "chp_your_api_key", // or set CHIRROP_API_KEY env var
...
The API key is resolved in order: explicit `Key` field, `CHIRROP_API_KEY` environment variable, `CHIRROP_API_KEY` from a `.env` file in the working directory.

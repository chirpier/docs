...
```bash
npm install @chirropjs/chirrop-js
```
...
The API key is resolved in order: explicit `key` parameter, `CHIRROP_API_KEY` environment variable, `CHIRROP_API_KEY` from a `.env` file in the working directory.

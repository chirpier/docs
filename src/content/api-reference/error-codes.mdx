...
Every error response from the ChirrOp API uses a standard JSON envelope. This page lists all error codes, their HTTP status codes, and common causes.
...
The per-token rate limit has been exceeded. ChirrOp applies a sliding-window rate limit per bearer token.

...
ChirrOp still uses `/destinations` in the HTTP route names, but conceptually these are reusable alert destinations.
...
| `email` | Not needed | Not needed (uses ChirrOp's mailer) |
...
```bash
TEST_ALERT_ID=$(curl -s -X POST "https://api.chirrop.com/v1.0/destinations/$DESTINATION_ID/test" \
  -H "Authorization: Bearer $CHIRROP_TOKEN" | jq -r '.alert_id')

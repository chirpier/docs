...
```go
_ = client.Log(ctx, chirpier.Log{Agent: "worker.billing", Event: "job.completed.count", Value: 1,
    Meta: map[string]any{"job_type": "invoice_generate", "queue": "billing"}})

_ = client.Log(ctx, chirpier.Log{Agent: "worker.billing", Event: "job.duration_ms", Value: 3200,
    Meta: map[string]any{"job_type": "invoice_generate"}})

_ = client.Log(ctx, chirpier.Log{Agent: "worker.billing", Event: "queue.backlog.gauge", Value: 42,

...
_ = client.Log(ctx, chirpier.Log{Agent: "openclaw.main", Event: "tool.errors.count", Value: 1, Meta: map[string]any{"tool_name": "browser.open", "workflow": "triage"}})
_ = client.Flush(ctx)
...
for _, eventDef := range events {
    if eventDef.Agent == "openclaw.main" && eventDef.Event == "tool.errors.count" {
        _, _ = client.GetEventAnalytics(ctx, eventDef.EventID, chirpier.AnalyticsWindowQuery{View: "window", Period: "1h", Previous: "previous_window"})
    }
}
```
...
Then continue with [Use chirpier-skills](/openclaw/skills), [Analytics Comparisons](/openclaw/analytics), and [Canonical Event Names](/openclaw/taxonomy).

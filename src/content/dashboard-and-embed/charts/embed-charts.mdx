...
ChirrOp provides hosted event embeds for rendering public event charts outside the main web app.

Typical flow:

1. Choose the public `eventId` to embed.
2. Optionally provide a `shareToken` if the embed should be scoped through a share link.
3. Render the hosted runtime in an iframe with `@chirropjs/charts` or `@chirropjs/embed`.
...
Use `@chirropjs/charts` for React apps and `@chirropjs/embed` for framework-agnostic usage.

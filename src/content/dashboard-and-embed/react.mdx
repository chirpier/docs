...
Use `@chirropjs/charts` when you want a React wrapper around the hosted ChirrOp event runtime.

## Install

```bash
npm install @chirropjs/charts
```
...
Use bare `<ChirrOpChart />` markup by default. Wrap it only when your app needs host-specific layout such as a card, title, placeholder copy, or custom empty state.
...
Use `React.useRef<ChirrOpChartRef>()` to access imperative methods:
...
- you want canonical ChirrOp page behavior in-product

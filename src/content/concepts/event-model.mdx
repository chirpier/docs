...
ChirrOp is built around flat numeric events.
...
When ChirrOp receives a log with an `agent` + `event` pair it has not seen before, it auto-creates an event definition. There is no manual registration step. The first log you send for a new event name creates the definition, and every subsequent log with the same identity appends to it.
